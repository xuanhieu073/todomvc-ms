...
public class DimissedReminderEnpoint : ICarterModule
{
    ...
    public class DimissedRemindersHandler(IMapper _mapper) : IRequestHandler<DimissedRemindersQuery, List<ReminderDto>>
    {
        public async Task<List<ReminderDto>> Handle(DimissedRemindersQuery request, CancellationToken cancellationToken)
        {
            var dimissedReminders = await DB.Find<Reminder>().Match(r => r.State == ReminderState.Dismissed && r.DimissAt >= request.from).ExecuteAsync();
            return _mapper.Map<List<ReminderDto>>(dimissedReminders);
        }
    }
}